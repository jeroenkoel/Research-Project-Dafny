// Dafny program hashMap.dfy compiled into C#
...
// Command-line arguments: run --no-verify c:\homeProjects\Dafny\hashMap\hashMap.dfy
// hashMap.dfy
...
module Hashing {
  ...
  class HashMap<K(==), V> {
    ...
    constructor (fnc: K -> int)
      ...
      ensures forall i: int {:trigger table[i]} :: 0 <= i < SIZE ==> table[i] == []
      ensures hashFnc == fnc
    {
      table := new seq<Entry<K, V>>[SIZE] ((i: nat) => []);
      hashFnc := fnc;
    }
    ...
    ghost predicate keyUniquenessChain(chain: seq<Entry<K, V>>)
      decreases chain
    {
      forall i: int, j: int {:trigger chain[j], chain[i]} :: 
        0 <= i < |chain| &&
        0 <= j < |chain| &&
        i != j ==>
    ...
    ghost predicate chainUniqueness(chain1: seq<Entry<K, V>>, chain2: seq<Entry<K, V>>)
      decreases chain1, chain2
    {
      forall i: int, j: int {:trigger chain2[j], chain1[i]} :: 
        0 <= i < |chain1| &&
        0 <= j < |chain2| ==>
          chain1[i].key != chain2[j].key
    }
      ...
      decreases {list}, list
    {
      forall i: int {:trigger list[i]} :: 
        0 <= i < list.Length ==>
          forall e: Entry?<K, V> {:trigger e.key} {:trigger e in list[i]} :: 
            e in list[i] ==>
              getIndex(e.key) == i
    }
    ...
    lemma keyNotInHashedChainNotInTable(key: K)
      ...
      ensures forall e: seq<Entry<K, V>> {:trigger keysOfChain(e)} {:trigger e in table[..]} :: e in table[..] ==> key !in keysOfChain(e)
    ...
    {
      var hashCode: int := hashFnc(key);
      hashCode % SIZE
    }

    function keysOfChain(chain: seq<Entry<K, V>>): seq<K>
      ensures forall e: K {:trigger e in keysOfChain(chain)} :: e in keysOfChain(chain) ==> exists e2: Entry<K, V> {:trigger e2.key} {:trigger e2 in chain} :: e2 in chain && e == e2.key
      ensures forall e: Entry<K, V> {:trigger e.key} {:trigger e in chain} :: e in chain ==> e.key in keysOfChain(chain)
      decreases chain
    {
      if |chain| == 0 then
        []
      else
        [chain[0].key] + keysOfChain(chain[1..])
    }
      ...
      decreases list
    {
      if |list| == 0 then
        ...
        keysOfChain(list[0]) + keysOfTable(list[1..])
    }

    lemma /*{:_inductionTrigger keysOfTable(list)}*/ /*{:_induction list}*/ keysOfTableCorrect(list: seq<seq<Entry<K, V>>>)
      ensures forall e1: seq<Entry<K, V>> {:trigger keysOfChain(e1)} {:trigger e1 in list} :: e1 in list ==> forall e2: K {:trigger e2 in keysOfTable(list)} {:trigger e2 in keysOfChain(e1)} :: e2 in keysOfChain(e1) ==> e2 in keysOfTable(list)
      ensures forall e1: K {:trigger e1 in keysOfTable(list)} :: e1 in keysOfTable(list) ==> exists e2: seq<Entry<K, V>> {:trigger keysOfChain(e2)} {:trigger e2 in list} :: e2 in list && e1 in keysOfChain(e2)
      decreases list
    {
      assert forall e1: K {:trigger e1 in keysOfTable(list)} :: e1 in keysOfTable(list) ==> exists e2: seq<Entry<K, V>> {:trigger keysOfChain(e2)} {:trigger e2 in list} :: e2 in list && e1 in keysOfChain(e2);
    }

    function getEntryChain(chain: seq<Entry<K, V>>, key: K): V
      requires key in keysOfChain(chain)
      requires keyUniquenessChain(chain)
      ensures forall e: Entry<K, V> {:trigger e.value} {:trigger e.key} {:trigger e in chain} :: e in chain && e.key == key ==> getEntryChain(chain, key) == e.value
      decreases chain
    {
      if |chain| == 1 then
        chain[0].value
      else if chain[0].key == key then
        chain[0].value
      else
        getEntryChain(chain[1..], key)
    }
    ...
    function replaceKey(entry: Entry<K, V>, chain: seq<Entry<K, V>>): seq<Entry<K, V>>
      ...
      ensures forall e: Entry<K, V> {:trigger e.key} {:trigger e in chain} :: e in chain ==> e.key in keysOfChain(replaceKey(entry, chain))
      ensures forall e: Entry<K, V> {:trigger e.key} {:trigger e in replaceKey(entry, chain)} :: e in replaceKey(entry, chain) ==> e.key in keysOfChain(chain)
      ensures forall e: Entry<K, V> {:trigger e in replaceKey(entry, chain)} {:trigger e.key} {:trigger e in chain} :: e in chain && e.key != entry.key ==> e in replaceKey(entry, chain)
      ensures forall e: Entry<K, V> {:trigger e in chain} {:trigger e in replaceKey(entry, chain)} :: e in replaceKey(entry, chain) && e != entry ==> e in chain
      decreases entry, chain
    {
      if |chain| == 1 then
        [entry]
      else if chain[0].key == entry.key then
        [entry] + chain[1..]
      else
        [chain[0]] + replaceKey(entry, chain[1..])
    }
    ...
    function addKey(entry: Entry<K, V>, chain: seq<Entry<K, V>>): seq<Entry<K, V>>
      ...
      ensures forall e: Entry<K, V> {:trigger e in addKey(entry, chain)} {:trigger e in chain} :: e in chain ==> e in addKey(entry, chain)
      ensures forall e: Entry<K, V> {:trigger e in chain} {:trigger e in addKey(entry, chain)} :: e in addKey(entry, chain) && e != entry ==> e in chain
      decreases entry, chain
    {
      chain + [entry]
    }
    ...
    method put(key: K, value: V)
      ...
      ensures keyUniquenessStrong(table[..])
      ensures keyMapsToHashedIndex(table)
      ensures forall e: int {:trigger table[e]} {:trigger old(table[e])} :: 0 <= e < SIZE && e != getIndex(key) ==> old(table[e]) == table[e]
      ensures key in keysOfTable(table[..])
      ensures key in keysOfChain(table[getIndex(key)])
      ensures value == getEntryChain(table[getIndex(key)], key)
    {
      var index := getIndex(key);
      var ent := new Entry<K, V>(key, value);
      if key in keysOfChain(table[index]) {
    ...
    function deleteKey(key: K, chain: seq<Entry<K, V>>): seq<Entry<K, V>>
      ...
      ensures forall e: Entry<K, V> {:trigger e in deleteKey(key, chain)} {:trigger e.key} {:trigger e in chain} :: e in chain && e.key != key ==> e in deleteKey(key, chain)
      ensures forall e: Entry<K, V> {:trigger e.key} {:trigger e in chain} {:trigger e in deleteKey(key, chain)} :: (e in deleteKey(key, chain) ==> e in chain) && (e in deleteKey(key, chain) ==> e.key != key)
      decreases chain
    {
      if |chain| == 1 then
        []
...
namespace Hashing {
  ...
  public abstract class Option<T> : _IOption<T> {
    ...
    public static Hashing._IOption<T> Default() {
      return create_None();
    }
    public static Dafny.TypeDescriptor<Hashing._IOption<T>> _TypeDescriptor() {
      return new Dafny.TypeDescriptor<Hashing._IOption<T>>(Hashing.Option<T>.Default());
    }
  ...
  public class Option_Some<T> : Option<T> {
    ...
    public override bool Equals(object other) {
      var oth = other as Hashing.Option_Some<T>;
  ...
  public class Option_None<T> : Option<T> {
    ...
    public override bool Equals(object other) {
      var oth = other as Hashing.Option_None<T>;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
  ...
  public partial class HashMap<K, V> {
    public HashMap() {
      this.table = new Dafny.ISequence<Hashing.Entry<K, V>>[0];
      this._hashFnc = ((K x0) => BigInteger.Zero);
    }
    public Dafny.ISequence<Hashing.Entry<K, V>>[] table {get; set;}
    public void __ctor(Func<K, BigInteger> fnc)
    {
      Func<BigInteger, Dafny.ISequence<Hashing.Entry<K, V>>> _init0 = ((System.Func<BigInteger, Dafny.ISequence<Hashing.Entry<K, V>>>)((_0_i) => {
        return Dafny.Sequence<Hashing.Entry<K, V>>.FromElements();
      }));
      Dafny.ISequence<Hashing.Entry<K, V>>[] _nw0 = new Dafny.ISequence<Hashing.Entry<K, V>>[Dafny.Helpers.ToIntChecked((this).SIZE, "array size exceeds memory limit")];
    ...
    public BigInteger getIndex(K key) {
      BigInteger _0_hashCode = Dafny.Helpers.Id<Func<K, BigInteger>>((this).hashFnc)(key);
      return Dafny.Helpers.EuclideanModulus(_0_hashCode, (this).SIZE);
    }
    public Dafny.ISequence<K> keysOfChain(Dafny.ISequence<Hashing.Entry<K, V>> chain) {
      Dafny.ISequence<K> _0___accumulator = Dafny.Sequence<K>.FromElements();
      var _this = this;
    TAIL_CALL_START: ;
      if ((new BigInteger((chain).Count)).Sign == 0) {
        return Dafny.Sequence<K>.Concat(_0___accumulator, Dafny.Sequence<K>.FromElements());
      } else {
        _0___accumulator = Dafny.Sequence<K>.Concat(_0___accumulator, Dafny.Sequence<K>.FromElements(((chain).Select(BigInteger.Zero)).key));
        Hashing.HashMap<K, V> _in0 = _this;
        Dafny.ISequence<Hashing.Entry<K, V>> _in1 = (chain).Drop(BigInteger.One);
        _this = _in0;
        ;
        chain = _in1;
    ...
    public Dafny.ISequence<K> keysOfTable(Dafny.ISequence<Dafny.ISequence<Hashing.Entry<K, V>>> list) {
      Dafny.ISequence<K> _0___accumulator = Dafny.Sequence<K>.FromElements();
      var _this = this;
    TAIL_CALL_START: ;
      if ((new BigInteger((list).Count)).Sign == 0) {
        ...
        _0___accumulator = Dafny.Sequence<K>.Concat(_0___accumulator, (_this).keysOfChain((list).Select(BigInteger.Zero)));
        Hashing.HashMap<K, V> _in0 = _this;
        Dafny.ISequence<Dafny.ISequence<Hashing.Entry<K, V>>> _in1 = (list).Drop(BigInteger.One);
        _this = _in0;
        ;
        list = _in1;
        goto TAIL_CALL_START;
    ...
    public V getEntryChain(Dafny.ISequence<Hashing.Entry<K, V>> chain, K key)
    {
      var _this = this;
    TAIL_CALL_START: ;
      if ((new BigInteger((chain).Count)) == (BigInteger.One)) {
        return ((chain).Select(BigInteger.Zero)).@value;
      } else if (object.Equals(((chain).Select(BigInteger.Zero)).key, key)) {
        return ((chain).Select(BigInteger.Zero)).@value;
      } else {
        Hashing.HashMap<K, V> _in0 = _this;
        Dafny.ISequence<Hashing.Entry<K, V>> _in1 = (chain).Drop(BigInteger.One);
    ...
    public Hashing._IOption<V> @get(K key)
    {
      Hashing._IOption<V> @out = Hashing.Option<V>.Default();
      BigInteger _0_index;
      _0_index = (this).getIndex(key);
      if (((this).keysOfChain((this.table)[(int)(_0_index)])).Contains(key)) {
        @out = Hashing.Option<V>.create_Some((this).getEntryChain((this.table)[(int)(_0_index)], key));
      } else {
        @out = Hashing.Option<V>.create_None();
      }
      return @out;
    }
    public Dafny.ISequence<Hashing.Entry<K, V>> replaceKey(Hashing.Entry<K, V> entry, Dafny.ISequence<Hashing.Entry<K, V>> chain)
    {
      Dafny.ISequence<Hashing.Entry<K, V>> _0___accumulator = Dafny.Sequence<Hashing.Entry<K, V>>.FromElements();
      var _this = this;
    TAIL_CALL_START: ;
      if ((new BigInteger((chain).Count)) == (BigInteger.One)) {
        return Dafny.Sequence<Hashing.Entry<K, V>>.Concat(_0___accumulator, Dafny.Sequence<Hashing.Entry<K, V>>.FromElements(entry));
      } else if (object.Equals(((chain).Select(BigInteger.Zero)).key, (entry).key)) {
        return Dafny.Sequence<Hashing.Entry<K, V>>.Concat(_0___accumulator, Dafny.Sequence<Hashing.Entry<K, V>>.Concat(Dafny.Sequence<Hashing.Entry<K, V>>.FromElements(entry), (chain).Drop(BigInteger.One)));
      } else {
        _0___accumulator = Dafny.Sequence<Hashing.Entry<K, V>>.Concat(_0___accumulator, Dafny.Sequence<Hashing.Entry<K, V>>.FromElements((chain).Select(BigInteger.Zero)));
        Hashing.HashMap<K, V> _in0 = _this;
        Hashing.Entry<K, V> _in1 = entry;
        Dafny.ISequence<Hashing.Entry<K, V>> _in2 = (chain).Drop(BigInteger.One);
        _this = _in0;
        ;
        entry = _in1;
        chain = _in2;
    ...
    public Dafny.ISequence<Hashing.Entry<K, V>> addKey(Hashing.Entry<K, V> entry, Dafny.ISequence<Hashing.Entry<K, V>> chain)
    {
      return Dafny.Sequence<Hashing.Entry<K, V>>.Concat(chain, Dafny.Sequence<Hashing.Entry<K, V>>.FromElements(entry));
    }
    public void put(K key, V @value)
    {
      BigInteger _0_index;
      _0_index = (this).getIndex(key);
      Hashing.Entry<K, V> _1_ent;
      Hashing.Entry<K, V> _nw0 = new Hashing.Entry<K, V>();
      _nw0.__ctor(key, @value);
      _1_ent = _nw0;
      if (((this).keysOfChain((this.table)[(int)(_0_index)])).Contains(key)) {
        Dafny.ISequence<Hashing.Entry<K, V>>[] _arr0 = this.table;
        _arr0[(int)((_0_index))] = (this).replaceKey(_1_ent, (this.table)[(int)(_0_index)]);
      } else {
        Dafny.ISequence<Hashing.Entry<K, V>>[] _arr1 = this.table;
        _arr1[(int)((_0_index))] = (this).addKey(_1_ent, (this.table)[(int)(_0_index)]);
      }
    }
    public Dafny.ISequence<Hashing.Entry<K, V>> deleteKey(K key, Dafny.ISequence<Hashing.Entry<K, V>> chain)
    {
      Dafny.ISequence<Hashing.Entry<K, V>> _0___accumulator = Dafny.Sequence<Hashing.Entry<K, V>>.FromElements();
      var _this = this;
    TAIL_CALL_START: ;
      if ((new BigInteger((chain).Count)) == (BigInteger.One)) {
        return Dafny.Sequence<Hashing.Entry<K, V>>.Concat(_0___accumulator, Dafny.Sequence<Hashing.Entry<K, V>>.FromElements());
      } else if (object.Equals(((chain).Select(BigInteger.Zero)).key, key)) {
        return Dafny.Sequence<Hashing.Entry<K, V>>.Concat(_0___accumulator, (chain).Drop(BigInteger.One));
      } else {
        _0___accumulator = Dafny.Sequence<Hashing.Entry<K, V>>.Concat(_0___accumulator, Dafny.Sequence<Hashing.Entry<K, V>>.FromElements((chain).Select(BigInteger.Zero)));
        Hashing.HashMap<K, V> _in0 = _this;
        K _in1 = key;
    ...
    public Hashing._IOption<V> delete(K key)
    {
      Hashing._IOption<V> @out = Hashing.Option<V>.Default();
      BigInteger _0_index;
      _0_index = (this).getIndex(key);
      if (((this).keysOfChain((this.table)[(int)(_0_index)])).Contains(key)) {
        @out = Hashing.Option<V>.create_Some((this).getEntryChain((this.table)[(int)(_0_index)], key));
        Dafny.ISequence<Hashing.Entry<K, V>>[] _arr0 = this.table;
        _arr0[(int)((_0_index))] = (this).deleteKey(key, (this.table)[(int)(_0_index)]);
      } else {
        @out = Hashing.Option<V>.create_None();
      }
      return @out;
    ...
    public BigInteger SIZE { get {
      return new BigInteger(512);
    } }
    public Func<K, BigInteger> _hashFnc {get; set;}
    public Func<K, BigInteger> hashFnc { get {
      return this._hashFnc;
    } }
  }

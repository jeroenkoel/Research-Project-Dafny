// Dafny program keyStore.dfy compiled into C#
...
// Command-line arguments: run --no-verify c:\homeProjects\Dafny\keyStore\keyStore.dfy
// keyStore.dfy
...
class KeyStore {
  ...
  function Put(store: map<string, object>, entry: map<string, object>): map<string, object>
    ensures forall k: seq<char> {:trigger k in Put(store, entry)} {:trigger k in entry} {:trigger k in store} :: k in store || k in entry ==> k in Put(store, entry)
    ensures forall k: seq<char> {:trigger entry[k]} {:trigger Put(store, entry)[k]} {:trigger k in entry} :: k in entry ==> Put(store, entry)[k] == entry[k]
    ensures forall k: seq<char> {:trigger store[k]} {:trigger Put(store, entry)[k]} {:trigger k in store} {:trigger k in entry} :: k !in entry && k in store ==> Put(store, entry)[k] == store[k]
    ensures forall k: seq<char> {:trigger k in Put(store, entry)} {:trigger k in store} {:trigger k in entry} :: k !in entry && k !in store ==> k !in Put(store, entry)
    decreases store, entry
  {
    map x: seq<char> {:trigger store[x]} {:trigger entry[x]} {:trigger x in entry} {:trigger x in store.Keys + entry.Keys} | x in store.Keys + entry.Keys :: if x in entry then entry[x] else store[x]
  }

  method put(key: string, value: object)
    modifies this
    ensures forall k: seq<char> {:trigger k in entries} {:trigger k in old(entries)} :: k in old(entries) || k == key ==> k in entries
    ensures entries[key] == value
    ensures forall k: seq<char> {:trigger old(entries[k])} {:trigger entries[k]} {:trigger k in old(entries)} :: k != key && k in old(entries) ==> entries[k] == old(entries[k])
    ensures forall k: seq<char> {:trigger k in entries} {:trigger k in old(entries)} :: k != key && k !in old(entries) ==> k !in entries
    decreases key, value
  {
    ...
    decreases key
  {
    if key in entries.Keys {
      val := Some(entries[key]);
      entries := map x: seq<char> {:trigger entries[x]} {:trigger x in entries.Keys - {key}} | x in entries.Keys - {key} :: entries[x];
    } else {
      val := None;
    }
  }
...
namespace _module {
  ...
  public partial class KeyStore {
    public KeyStore() {
      this.entries = Dafny.Map<Dafny.ISequence<Dafny.Rune>, object>.Empty;
    }
    public Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object> entries {get; set;}
    public void __ctor()
    {
      (this).entries = Dafny.Map<Dafny.ISequence<Dafny.Rune>, object>.FromElements();
    }
    public _IOption<object> @get(Dafny.ISequence<Dafny.Rune> key)
    {
      _IOption<object> val = Option<object>.Default();
      if (((this.entries).Keys).Contains(key)) {
        val = _module.Option<object>.create_Some(Dafny.Map<Dafny.ISequence<Dafny.Rune>, object>.Select(this.entries,key));
      } else {
        val = _module.Option<object>.create_None();
      }
      return val;
    }
    public Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object> Put(Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object> store, Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object> entry)
    {
      return Dafny.Helpers.Id<Func<Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object>, Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object>, Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object>>>((_0_store, _1_entry) => ((System.Func<Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object>>)(() => {
        var _coll0 = new System.Collections.Generic.List<Dafny.Pair<Dafny.ISequence<Dafny.Rune>,object>>();
        foreach (Dafny.ISequence<Dafny.Rune> _compr_0 in (Dafny.Set<Dafny.ISequence<Dafny.Rune>>.Union((_0_store).Keys, (_1_entry).Keys)).Elements) {
          Dafny.ISequence<Dafny.Rune> _2_x = (Dafny.ISequence<Dafny.Rune>)_compr_0;
          if ((Dafny.Set<Dafny.ISequence<Dafny.Rune>>.Union((_0_store).Keys, (_1_entry).Keys)).Contains(_2_x)) {
            _coll0.Add(new Dafny.Pair<Dafny.ISequence<Dafny.Rune>,object>(_2_x, (((_1_entry).Contains(_2_x)) ? (Dafny.Map<Dafny.ISequence<Dafny.Rune>, object>.Select(_1_entry,_2_x)) : (Dafny.Map<Dafny.ISequence<Dafny.Rune>, object>.Select(_0_store,_2_x)))));
          }
        }
        return Dafny.Map<Dafny.ISequence<Dafny.Rune>,object>.FromCollection(_coll0);
      }))())(store, entry);
    }
    public void put(Dafny.ISequence<Dafny.Rune> key, object @value)
    {
      Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object> _0_update;
      _0_update = Dafny.Map<Dafny.ISequence<Dafny.Rune>, object>.FromElements(new Dafny.Pair<Dafny.ISequence<Dafny.Rune>, object>(key, @value));
      (this).entries = (this).Put(this.entries, _0_update);
    }
    public _IOption<object> delete(Dafny.ISequence<Dafny.Rune> key)
    {
      _IOption<object> val = Option<object>.Default();
      if (((this.entries).Keys).Contains(key)) {
        val = _module.Option<object>.create_Some(Dafny.Map<Dafny.ISequence<Dafny.Rune>, object>.Select(this.entries,key));
        (this).entries = Dafny.Helpers.Id<Func<Dafny.ISequence<Dafny.Rune>, Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object>>>((_0_key) => ((System.Func<Dafny.IMap<Dafny.ISequence<Dafny.Rune>,object>>)(() => {
          var _coll0 = new System.Collections.Generic.List<Dafny.Pair<Dafny.ISequence<Dafny.Rune>,object>>();
          foreach (Dafny.ISequence<Dafny.Rune> _compr_0 in (Dafny.Set<Dafny.ISequence<Dafny.Rune>>.Difference((this.entries).Keys, Dafny.Set<Dafny.ISequence<Dafny.Rune>>.FromElements(_0_key))).Elements) {
            Dafny.ISequence<Dafny.Rune> _1_x = (Dafny.ISequence<Dafny.Rune>)_compr_0;
            if ((Dafny.Set<Dafny.ISequence<Dafny.Rune>>.Difference((this.entries).Keys, Dafny.Set<Dafny.ISequence<Dafny.Rune>>.FromElements(_0_key))).Contains(_1_x)) {
              _coll0.Add(new Dafny.Pair<Dafny.ISequence<Dafny.Rune>,object>(_1_x, Dafny.Map<Dafny.ISequence<Dafny.Rune>, object>.Select(this.entries,_1_x)));
            }
          }
          return Dafny.Map<Dafny.ISequence<Dafny.Rune>,object>.FromCollection(_coll0);
        }))())(key);
